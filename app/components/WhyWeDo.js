...
export const whyTexts = [
  'Cyclop is different, fast and hassle free.',
  'Cyclop is made to fit.',
  'Cyclop is reliable.',
  'Cyclop is ready to assist always.',
  'Cyclop spells success for your company.',
  'Switch to Cyclop.',
];
  ...
  return (
    <>
      <div className='flex items-center justify-between w-[90%] lg:w-[80%] xl:w-[75%] mt-20'>
        ...
        <p
          className={`${rubik.className} underline max-lg:text-sm max-sm:text-xs hidden max-sm:block cursor-pointer`}
        >
          {'See all>'}
        </p>
      </div>
      <div className='relative flex overflow-hidden w-[99%] md:w-[99%] xl:w-[99%] group'>
        <div className='flex gap-2 mt-10 animate-marquee group-hover:pause'>
          {whyTexts.map((text, index) => (
            <WhyCard key={index} text={text} />
          ))}
        </div>
